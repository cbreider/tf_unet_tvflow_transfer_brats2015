...
class Trainer(object):
    ...
    def __init__(self, net, data_provider_train, data_provider_val, out_path, train_config, mode,  restore_path=None,
        ...
        :param data_provider_test: callable returning training and test data
    ...
    def _get_optimizer(self, global_step, vars=None, lr=None):
        if vars is None:
    ...
    def _initialize(self):
        ...
        :returns init: tf global variable initializer
        """
        ...
        with tf.Session() as sess:
            ...
            try:
                for step in range(init_step, (self._n_epochs * self._training_iters) + 1):
                    ...
                    if (
                            s_train * self.config.batch_size_train + self.config.batch_size_train) > self.data_provider_train.size:
                        sess.run(self.data_provider_train.init_op)
                        s_train = 0
                    ...
                    avg_score_vals_batch.append(
                        [loss, cs, err, acc, iou, dice, d_complete, d_core, d_enhancing, l1, l2])
                            ...
                            if self._unfreeze_all_layers_epochs:
                                if self._unfreeze_all_layers_epochs and self._unfreeze_all_layers_epochs >= 0:
                                    if (epoch - last_best_validation_scores[0]) >= self._unfreeze_all_layers_epochs:
                                        ...
                                        self.optimizer_op = self._get_optimizer(self.global_step,
                                                                                vars=self.net.variables,
                                                                                lr=lr)
                                ...
                                if self._early_stopping_epochs >= 0 and (
