...
if __name__ == "__main__":
    ...
    data_paths = DataPaths(data_path="default", mode="TVFLOW_REGRESSION")
    data_paths.load_data_paths(mkdirs=True, restore_dir=restore_path if restore_mode == 1 else None)
    ...
    file_paths = TrainingDataset(paths=data_paths,
                                 mode=TrainingModes.TVFLOW_REGRESSION,
    ...
    net = unet.Unet(n_channels=config.DataParams.nr_of_channels,
                    ...
                    tv_regularizer=config.ConvNetParams.tv_regularizer)
