...
def get_tv_smoothed(img, tau, weight, eps, m_itr):
    with tf.name_scope("tv_smoothing"):
        u = tf.zeros_like(img)
...
def reshape_clusters_to_cluster_number(clusters, n_clusters):
    with tf.name_scope("reshape_clusters"):
...
def get_image_summary(img, idx=0):
    ...
    :param img: input 4d tensor
    :param idx: idnex of the image to choose
